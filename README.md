...
2. Implement feed generation logic in `src/algos`
   ...
   For inspiration, we've provided a very simple feed algorithm (`espresso-vibes`) that returns all posts related to the titular character of the TV show ALF.
...
Install dependencies with `yarn` and then run the server with `yarn start`. This will start the server on port 3000, or what's defined in `.env`. You can then watch the firehose output in the console and access the output of the default custom ALF feed at [http://localhost:3000/xrpc/app.bsky.feed.getFeedSkeleton?feed=at://did:example:alice/app.bsky.feed.generator/espresso-vibes](http://localhost:3000/xrpc/app.bsky.feed.getFeedSkeleton?feed=at://did:example:alice/app.bsky.feed.generator/espresso-vibes).
